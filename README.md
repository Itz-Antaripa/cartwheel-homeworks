...
## Homework 3 executable workflow

The scenario skill is a reviewed procedure, not an importable generator. After
your coding agent writes the final JSONL, validate it before spending model
tokens:
...
The command checks the two expected schemas, unique identifiers, 175/75
group counts, three-turn limit, and five challenge scenarios for each of the
six documented dirty records. The runner repeats the per-record checks before
opening an HTTP session:

```bash
uv run python -m scenarios.runner scenarios/support_scenarios.jsonl \
  --model "$CARTWHEEL_MODEL" --output scenarios/final-results.jsonl
```

The explicit output path avoids the gitignored scratch directory. After the
run, export the complete Langfuse records, including observations and metadata:
...
The exporter fails when any of the 250 scenario identifiers has no trace. Use
`--allow-missing` only to diagnose an incomplete run. The Langfuse stack,
model runs, smoke SQL, and export require Docker plus provider credentials;
scenario validation and the complete test suite remain offline.

Save the complete smoke report with the fixed query file:
